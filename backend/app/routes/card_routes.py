...
from ..models.models import FavoriteCard
from ..schemas.schemas import FavoriteCardSchema
...
def get_card_blueprint():
    ...
    def _json_list(value):
        """Return a JSON text/list column as a list of strings."""
        if not value:
            return []
        ...
        try:
            ...
            logger.warning("卡片列表字段不是有效JSON，返回空列表")
            return []
    ...
    def _dump_card(card):
        ...
        result['key_points'] = _json_list(card.key_points)
        result['quotes'] = _json_list(card.quotes)
        return result

    def _dump_cards(cards):
        return [_dump_card(card) for card in cards]
    ...
    def get_all_cards():
        ...
        return jsonify(_dump_cards(cards))
    ...
    def get_card(id):
        ...
        return jsonify(_dump_card(card))
    ...
    def get_popular_cards():
        ...
        return jsonify(_dump_cards(popular_cards))
    ...
    def get_card_stats():
        ...
        results = _dump_cards(recent_cards)
